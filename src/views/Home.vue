  ...
  <div>
    <header class="home-header wrap" :class="{'active' : headerScroll}">
        ...
        <div class="header-search">
            <span class="app-name">waiting</span>
    ...
    <div class="category-list">
        <div v-for="item in categoryList" :key="item.categoryId" @click="tips">
            <img :src="$filters.prefix(item.imgUrl)" :alt="item.name" />
            <span>{{item.name}}</span>
        </div>
    </div>
    </van-skeleton>
    ...
    <div class="good" style="padding-bottom: 100px;">
        <van-skeleton title :row="3" :loading="loading" >
...
import { getLocal } from '@/common/js/untils'
    ...
    setup(){
      const router = useRouter()
        ...
        const goToDetail = (product) => {
          router.push({path: `/product/${product.goodsId}`})
        }
        nextTick(()=>{//nextTick 当数据更新了，在dom中渲染后，自动执行该函数，
