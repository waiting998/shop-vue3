        ...
        <div class="content">
            <van-pull-refresh v-model="refreshing" @refresh="onRefresh" class="product-list-refresh">
                <van-list
                    ...
                    :finished="finished"
                    :finished-text="productList.length ? '已经到底啦' : '搜索想要的东西'"
                    @load="onLoad"
                    @offset="10"
                >
                <template v-if="productList.length">
                    <div class="product-item" v-for="(item,index) in productList" :key="index" @click="productDetail(item)">
                        <img :src="$filters.prefix(item.goodsCoverImg)" :alt="item.name"/>
...
export default {
    ...
    setup(){
        ...
        const state = reactive({
            productList:[],
            keyword: route.query.keyword || '',  //route.query获取地址栏路由参数
            refreshing: false,
            loading: false,
            finished: false,
            orderBy:'',
        ...
        const init = async () => {
            ...
            // { data , data: { list } }将返回的数据解构赋值 方便快捷
